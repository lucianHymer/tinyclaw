...
2. **Inconsistent timestamp format**: `tinyclaw.sh` and `heartbeat-cron.sh` use `date '+%Y-%m-%d %H:%M:%S'`, while `create-dev-container.sh` and `remove-dev-container.sh` use `date -Is`.
